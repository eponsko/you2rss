...
class UpdatePodcasts():
            ...
            if mime == 'application/octet-stream' or mime == 'audio/mpeg':
                ...
                if data:
                    ...
                    pod = podcast.pod_set.create(pod_id=data['fileURL'], title_text=title,
                                                 ...
                                                 http_link="http://TODO")
                    log.info("Created pod " + str(pod))
                    pod.save()
                else:
                    log.info("mp3data() returned no data")
    ...
    def delete_podcast(self, title):
        channel = Podcast.objects.get(title_text=title)
        if channel:
            channel.delete()
...
if __name__ == '__main__':
    ...
    deleteid = subpodcast_parser.add_parser('deleteid')
    deleteid.add_argument('NAME', help="id of the podcast to delete")
    ...
    if args.command == 'channel':
        ...
        if args.action == 'updateall':
           ...
           # a.update_all_videos()
            ...
            log.info("Checking latest in podcasts")
           # a.check_latest_in_channels()
        elif args.action == 'deleteall':
            print("delete all podcasts")      
        elif args.action == 'updatestatic':
            log.info("checking static podcasts")
            a.update_static_podcasts()
        elif args.action == 'deleteid':
            log.info("trying to delete podcast with id"+str(args.NAME))
            a.delete_podcast(args.NAME)
